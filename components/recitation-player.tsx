...
export function RecitationPlayer({ recitations }: { recitations: RecitationMeta[] }) {
  ...
  return <View style={{ backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1, borderRadius: 22, padding: 16, marginBottom: 15 }}>
    ...
    <Pressable onPress={togglePlayback} style={({ pressed }) => ({ backgroundColor: colors.primary, marginTop: 15, borderRadius: 16, minHeight: 48, flexDirection: "row-reverse", alignItems: "center", justifyContent: "center", gap: 8, opacity: pressed ? 0.72 : 1 })}><MaterialIcons name={status.playing ? "pause" : "play-arrow"} size={22} color={colors.onPrimary} /><Text style={{ color: colors.onPrimary, fontSize: 14, fontWeight: "800" }}>{status.playing ? "إيقاف مؤقت" : pendingPlay || !status.isLoaded ? "جار تجهيز التلاوة" : "تشغيل التلاوة"}</Text></Pressable>
