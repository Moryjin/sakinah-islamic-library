...
export default function AdhkarScreen() {
  ...
  return <ScreenContainer className="px-5"><MotionIn className="flex-1"><FlatList data={adhkar} numColumns={2} columnWrapperStyle={adhkar.length ? { gap: 10 } : undefined} keyExtractor={(item) => item.id} renderItem={({ item }) => <Pressable onPress={() => router.push(`/reader/${item.id}` as Href)} style={({ pressed }) => ({ flex: 1, minHeight: 138, padding: 14, borderRadius: 18, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface, opacity: pressed ? 0.68 : 1 })}><View style={{ flexDirection: "row-reverse", justifyContent: "space-between", alignItems: "center" }}><View style={{ width: 33, height: 33, borderRadius: 11, backgroundColor: `${colors.primary}16`, alignItems: "center", justifyContent: "center" }}><MaterialIcons name={item.kind === "daily-ward" ? "auto-stories" : "wb-sunny"} size={18} color={colors.primary} /></View><MaterialIcons name="chevron-left" size={18} color={colors.muted} /></View><Text numberOfLines={2} style={{ color: colors.foreground, fontSize: 13, fontWeight: "800", marginTop: 11, ...rtl }}>{item.title}</Text><Text numberOfLines={3} style={{ color: colors.muted, fontSize: 10, lineHeight: 15, marginTop: 5, ...rtl }}>{item.subtitle}</Text><Text style={{ color: colors.primary, fontSize: 10, fontWeight: "700", marginTop: 8, ...rtl }}>مادة موثقة</Text></Pressable>} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingTop: 16, paddingBottom: 28, gap: 10 }} ListHeaderComponent={<>
    ...
    <View className="flex-row-reverse flex-wrap justify-between" style={{ gap: 9 }}>{categories.map((category) => <Pressable key={category.title} onPress={() => router.push("/adhkar/online" as Href)} style={({ pressed }) => ({ width: "48.7%", minHeight: 68, flexDirection: "row-reverse", alignItems: "center", gap: 8, backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1, borderRadius: 15, padding: 11, opacity: pressed ? 0.68 : 1 })}><MaterialIcons name={category.icon} size={18} color={colors.primary} /><Text style={{ color: colors.foreground, fontSize: 12, fontWeight: "700", flex: 1, ...rtl }}>{category.title}</Text></Pressable>)}</View>
