    ...
    <ScreenContainer className="px-5">
      <FlatList data={items} keyExtractor={(item) => item.id} renderItem={({ item }) => <LibraryItemCard item={item} />} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingTop: 16, paddingBottom: 28 }} ListHeaderComponent={<>
        ...
        <View style={{ flexDirection: "row-reverse", gap: 10, marginBottom: 14 }}>
          <TouchableOpacity onPress={() => router.push("/bukhari" as Href)} style={({ pressed }) => ({ flex: 1, minHeight: 92, padding: 14, borderRadius: 18, backgroundColor: colors.primary, opacity: pressed ? 0.72 : 1 })}><MaterialIcons name="menu-book" color="#FFFFFF" size={21} /><Text style={{ color: "#FFFFFF", fontWeight: "800", fontSize: 14, marginTop: 9, writingDirection: "rtl", textAlign: "right" }}>صحيح البخاري كاملًا</Text><Text style={{ color: "#DDF5EA", fontSize: 10, marginTop: 4, writingDirection: "rtl", textAlign: "right" }}>نص عربي · ٩٧ كتابًا</Text></TouchableOpacity>
          <TouchableOpacity onPress={() => router.push("/muslim" as Href)} style={({ pressed }) => ({ flex: 1, minHeight: 92, padding: 14, borderRadius: 18, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface, opacity: pressed ? 0.72 : 1 })}><MaterialIcons name="auto-stories" color={colors.warning} size={21} /><Text style={{ color: colors.foreground, fontWeight: "800", fontSize: 14, marginTop: 9, writingDirection: "rtl", textAlign: "right" }}>صحيح مسلم كاملًا</Text><Text style={{ color: colors.muted, fontSize: 10, marginTop: 4, writingDirection: "rtl", textAlign: "right" }}>نص عربي · مصدر مفتوح</Text></TouchableOpacity>
        </View>
