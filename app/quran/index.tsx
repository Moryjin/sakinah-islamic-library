...
import { QuranPackDownload } from "@/components/quran-pack-download";
  ...
  return (
    <ScreenContainer className="px-5" edges={["top", "bottom", "left", "right"]}>
      <FlatList
        ...
        renderItem={({ item }) => <Pressable onPress={() => router.push({ pathname: "/quran/[number]", params: { number: String(item.number) } })} style={({ pressed }) => ({ flex: 1, minHeight: 112, backgroundColor: colors.surface, borderRadius: 17, padding: 13, borderWidth: 1, borderColor: colors.border, opacity: pressed ? 0.7 : 1 })}><View style={{ flexDirection: "row-reverse", alignItems: "center", justifyContent: "space-between" }}><View style={{ width: 34, height: 34, borderRadius: 11, backgroundColor: `${colors.primary}14`, alignItems: "center", justifyContent: "center" }}><Text style={{ color: colors.primary, fontWeight: "800", fontSize: 12 }}>{item.number}</Text></View><MaterialIcons name="chevron-left" color={colors.muted} size={19} /></View><Text numberOfLines={1} style={{ color: colors.foreground, fontSize: 15, fontWeight: "800", marginTop: 11, ...rtl }}>{item.name}</Text><Text style={{ color: colors.muted, fontSize: 10, marginTop: 4, ...rtl }}>{item.numberOfAyahs} آيات · {item.revelationType === "Meccan" ? "مكية" : "مدنية"}</Text></Pressable>}
      />
    </ScreenContainer>
  );
}
